...
async def on_shutdown() -> None:
    ...
    # Do not delete the webhook on shutdown. During a Render deploy, the old
    # process can shut down after the new process has already set the webhook.
    # Deleting it here would leave Telegram without a webhook.
    await bot.session.close()


@app.get("/health")
async def health() -> dict:
    return {"status": "ok", "ai_configured": bool(settings.openrouter_api_key), "database": bool(settings.database_url)}
